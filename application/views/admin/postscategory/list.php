				...
				<div class="card">
					<div class="content">
						<div class="body collapse in" style="margin: 0 0 15px">
							<a href="<?=site_url('admin/postscategory/add')?>" class="btn btn-info btn-fill btn-wd">Thêm mới</a>
						</div>
								...
								<table class="table table-striped table-bordered" id="sample_1">
									...
									<?php if (!empty($result)) foreach($result as $item){?>
										<tr class="odd gradeX">
											...
											?></td>
											<td><?=@$item['alias']?></td>
											<td style="text-align: center"><a href="<?=@base_url('admin/postscategory/edit/'.$item['id'])?>"><i class="fa fa-pencil"></i> Sửa</a> | <a href="<?=@base_url('admin/postscategory/delete/'.$item['id'])?>" onclick="return confirm('Bạn có chắc chắn muốn xóa không?')" ><i class="fa fa-trash"></i> Xóa</a></td>
									<?php } ?>
									</tbody>
								</table>
							</div>
