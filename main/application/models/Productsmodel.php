...
class ProductsModel extends MY_Model {
	...
	public function getSearchProducts($type,$title,$categoryid,$limit=10,$offset){
		$this->db->select('products.*');
		...
		$this->db->or_like('products.sku', $title);
		$this->db->or_like('products.short_description', $title);
	...
	public function getProductName() {
		$this->db->select('products.title,products.id,products.sku');
		$query = $this->db->get('products');
		if($query->num_rows() > 0)  {
			$data = $query->result();
			return $data;
