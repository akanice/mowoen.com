				...
				<div class="card">
					<div class="content">
						<div class="body collapse in" style="margin: 0 0 15px">
							<a href="<?=site_url('admin/posts/add')?>" class="btn btn-info btn-fill btn-wd">Thêm mới</a>
						</div>
										...
										<tr>
											...
											<td>
												<select class="form-control" name="category">
													...
													<?php foreach ($postscategory as $c) {?>
										...
										<tr class="odd gradeX">
											...
											<td>
												<?php foreach (json_decode($item->categoryid) as $c) {
													$c_data = $this->postscategorymodel->read(array('id'=>$c),array(),true);?>
													<a href="<?=base_url('admin/postscategory/edit/'.$c_data->id)?>">
														<?php
															echo $c_data->title;
														?>
													</a>
											...
											<td style="text-align: center"><a href="<?=@base_url('admin/posts/edit/'.$item->id)?>"><i class="fa fa-pencil"></i> Sửa</a> | <a href="<?=@base_url('admin/posts/delete/'.$item->id)?>" onclick="return confirm('Bạn có chắc chắn muốn xóa không?')" ><i class="fa fa-trash"></i> Xóa</a></td>
										</tr>
										<?php } ?>
									</tbody>
								</table>
