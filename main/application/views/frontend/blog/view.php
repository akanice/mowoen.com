						...
						<div class="single-post-meta clearfix">
							...
							<div class="post-meta-item post-likes">
								<span class="sl-wrapper">
									<a href="#" class="sl-button" title="Like"><span class="sl-icon liked"></span><span class="sl-count"><?=@$meta_data['likes']?></span></a>
					...
					<div class="nav-post-links d-flex justify-content-between align-items-center clearfix">
						...
						<div class="prev-post nav-post">
							...
							<a href="<?=base_url($prevRow->alias)?>" class="nav-post-link d-flex justify-content-start align-items-center ratio_square position-relative">
								...
								<span class="nav-post-info"><span class="nav-post-title"><?=@$prevRow->title?></span><span class="nav-post-date"><?php echo date_format(date_create($prevRow->create_time),"d/m/y"); ?></span></span>
							</a>
							<?php } ?>
						</div>
						<div class="next-post nav-post">
							...
							<a href="<?=base_url($nextRow->alias)?>" class="nav-post-link d-flex justify-content-end align-items-center ratio_square position-relative">
								<span class="nav-post-text"><?=@$nextRow->title?></span>
						...
						<div class="wrapper">
							...
							<?php if ($related) { foreach ($related as $item) {?>
								<article class="col item post-item related-item">
									<div class="post-media post-grid-media ratio43_square"><a href="<?=base_url('chuyen-muc/'.@$item->cat_data->alias)?>" class="pic bg-size fix-size" style="background-image:url('<?=@$item->thumb?>')"></a></div>
									<div class="post-info">
										<div class="post-info-header d-flex justify-content-between align-items-center">
											<h3 class="post-title"><a href="<?=base_url('chuyen-muc/'.@$item->cat_data->alias)?>"><?=@$item->title?></a></h3>
		...
		<script>
			$(document).ready(function() {
				$('.sl-button').click(function(e) {
					e.preventDefault();
					count_like = parseInt($(this).find('.sl-count').html());
					$(this).toggleClass('active');
					if ($(this).hasClass('active')) {
						$(this).find('.sl-count').html(count_like+1);
					} else {
						$(this).find('.sl-count').html(count_like-1);
					}
				})
