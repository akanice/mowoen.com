...
class Customers extends MY_Controller{
    ...
    public function index(){
		$user_group = $this->adminsmodel->read(array('id'=>$this->session->userdata('adminid')),array(),true)->group;
        if ($user_group == 'admin') {
			$this->data['title']    = 'Quản lý data khách hàng';
	...
	private function saveData() {
		$data=array(
			...
			'phone' => ($this->input->post('f_phone')? $this->input->post('f_phone'): ''),
			'address' => ($this->input->post('f_address')? $this->input->post('f_address'): ''),
			'email' => ($this->input->post('f_email')? $this->input->post('f_email'): ''),
			'form_code' => ($this->input->post('form_code')? $this->input->post('form_code'): ''),
		);
		$this->customersmodel->create($data);
	}

	public function form_success() {
		$this->data['title'] = 'Gửi thông tin thành công';
		$this->data['temp'] = 'frontend/template/form_success';
